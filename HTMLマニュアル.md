...
本テンプレートはHTML, CSS, JavaScriptを使って作成していますが、未経験者でも扱えるように基本的な部分はHTMLを差し替える、関連箇所をコピー＆ペーストするといった簡単な作業で作成・運用が可能な設計にしてあります。デザインや機能の追加変更を希望される場合は、有償にてカスタマイズのご依頼も承りますので、ご希望の方は[こちら](https://)からお問い合わせください。

なお、本マニュアルはGitHubpagesを使って公開する前提で作成しています。GitHubpagesの環境構築がお済みでない方は[こちら](https://)を参照してください。

## ステップ1: テキストエディタを用意する

HTMLの編集自体はWindowsに標準搭載されているメモ帳等でも可能です。  
しかし、メモ帳では使い勝手がよくないので **テキストエディタ** を使って作業することをオススメします。様々な種類のフリーソフトが公開されていますので「テキストエディタ」で検索してみて、よさそうなものをダウンロードして使ってみてください。
...
headでは、検索したときに出てくるディスクリプション、ブラウザに表示されるファビコン、Xなどで共有したときに表示されるOGPなどの処理を行っています。  
...
一つのニュースブロックは`<li class="news-item" data-more="">`と`</li>`に囲まれた部分です。ニュースの件数を増やしたい場合にはこの部分をコピー＆ペーストして編集し、古いニュースを消したい場合はこの部分を削除してください。ニュースはhtmlに記載された順番に表示されます。  

- 日付、内容を編集する  
    日付は`<div class="news-date">`と`</div>`、内容は`<div class="news-title">`と`</div>`に囲まれている部分を編集してください。
    <!--
    単にテキストを編集すれば足りるところは画像を貼るだけでもいいのだろうか？supersuperさんからもらったやつのようにコードの中に解説を埋め込むやり方はちょっとわかりにくいような気もするが…あるいはサンプルのドキュメント自体を「ここに氏名をいれてください」みたいにするとか？
    -->
...
- 外部リンクを編集する
    ...
    「記事を見る」の表記を消したい場合は`<a href="#" target="_blank" class="link">`から`</a>`までを削除してください。  

<!--
※なぜか一番上の記事を見るが表示されていないが…おまかん？
-->

### 研究テーマを編集する

`<!-- START 研究テーマ -->`と`<!-- END 研究テーマ -->`に囲まれている部分です。  
ここでは研究テーマの見出しと内容の編集を行えます。
...
- 年を入れた表記
    ...
    同年の項目を増やしたい場合は`<p>日本データサイエンス賞受</p>`の部分を追加してください。  
...
jpgとpngはWindowsのペイント等で作成編集できますが、svgやicoの編集はWindows標準搭載のアプリケーションではできません。無料でsvgやicoに変換できるツールがありますので、インターネットで「png svg 変換ツール」「png ico 変換ツール」などと検索して探してみてください。  
...
<!--
差し替えてくださいではなく、差し替えもしくは削除してくださいとしたいところだが…画像をそのまま削除するだけだとエラーが吐き出されるよなぁ…説明めんどいなぁ…
-->
...
# 応用編
<!-- #は一文書に一個なので、これをするなら　## 基本編　と　##応用編　であるべき-->

## 独自ドメインで運用したい

## github.io以下のURLを変えたい

## 参考ページ

- [Githubpages](https://docs.github.com/ja/pages/getting-started-with-github-pages "Github pages")
- [Githubのドキュメント](https://docs.github.com/ja/get-started "Githubのドキュメント")
