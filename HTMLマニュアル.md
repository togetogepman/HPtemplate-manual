...
本テンプレートはHTML, CSS, JavaScriptを使って作成していますが、未経験者でも扱えるように基本的な部分はHTMLを差し替える、関連箇所をコピー＆ペーストするといった簡単な作業で作成・運用が可能な設計にしてあります。デザインや機能の追加変更を希望される場合は、有償にてカスタマイズのご依頼も承りますので、ご希望の方は[こちら](https://www.academeia15.co.jp/contact)からお問い合わせください。
...
> [!TIP]
> OGPを適切に設定するにはページのURLと画像のURLを「絶対パス」で記述する必要があります。  
> その場合は`<meta property="og:url" content="" />`と`<meta property="og:image" content="./assets/images/common/ogp.jpg" />`の`content=""`の部分をそれぞれホームページのURLとOGP画像の絶対パスで指定してください。
> もっとも、ここは変更しなくても支障なく運用することは可能です。  
...
- 外部リンクを編集する
    ...
    「記事を見る」の表記を消したい場合は`<div class="news-text is-link">`の箇所を`<div class="news-text">`に変更してください。  
